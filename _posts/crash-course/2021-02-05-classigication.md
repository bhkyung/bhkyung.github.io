...
참고: 로지스틱 회귀의 임계값 '조정'은 학습률과 같은 초매개변수 조정과 다릅니다. 어떤 임계값을 선택하는가에 따라 실수로 인해 받는 피해의 양이 달라집니다. 예를 들어 스팸이 아닌 메일을 스팸으로 잘못 라벨을 지정하면 피해가 매우 큽니다. 하지만 스팸 메일을 스팸 아님으로 잘못 라벨을 지정하면 불편하긴 하지만 치명적인 피해는 아닙니다. {: .notice--info}
